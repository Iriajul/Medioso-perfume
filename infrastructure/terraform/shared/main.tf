# ─── Mad Perfume Main Terraform Configuration ───────────────────────────
...
resource "aws_s3_bucket_cors_configuration" "media" {
  ...
  cors_rule {
    ...
    allowed_origins = ["https://${var.host_prefix}api.${var.domain}", "https://${var.host_prefix}admin.${var.domain}", "http://localhost:8000", "http://localhost:3000"]
    expose_headers  = ["ETag"]
...
resource "aws_security_group" "app_sg" {
  ...
  # RDS — PostgreSQL only from resources in this same security group (the EC2 app server)
  ingress {
    from_port   = 5432
    to_port     = 5432
    protocol    = "tcp"
